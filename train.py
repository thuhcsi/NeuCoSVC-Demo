...
from dataset.dataset import (LogMelSpectrogram, MelDataset, get_dataset_filelist,
                         mel_spectrogram)
from modules.models import (GeneratorNSF, MultiPeriodDiscriminator,
                     MultiScaleDiscriminator)
from modules.loss import (discriminator_loss, feature_loss,
                     generator_loss)
from utils.tools import (AttrDict, build_env, plot_spectrogram,
...
def train(rank, a, h):
    ...
    if rank == 0:
        ...
        print("checkpoints directory : ", a.ckpt_dir)

    if os.path.isdir(a.ckpt_dir):
        cp_g = scan_checkpoint(a.ckpt_dir, 'g_')
        cp_do = scan_checkpoint(a.ckpt_dir, 'do_')
    ...
    if rank == 0:
        ...
        sw = SummaryWriter(os.path.join(a.ckpt_dir, 'logs'))
        ...
        for i, batch in pb:
            ...
            if rank == 0:
                ...
                if steps % a.checkpoint_interval == 0 and steps != 0:
                    checkpoint_path = "{}/g_{:08d}.pt".format(a.ckpt_dir, steps)
                    save_checkpoint(checkpoint_path,
                                    {'generator': (generator.module if h.num_gpus > 1 else generator).state_dict()})
                    checkpoint_path = "{}/do_{:08d}.pt".format(a.ckpt_dir, steps)
                    save_checkpoint(checkpoint_path, 
                ...
                if steps % a.validation_interval == 0 and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in progress_bar(enumerate(validation_loader), total=len(validation_loader), parent=mb):
                            x, f0, pitch, y, _, y_mel = batch
                            y_g_hat = generator(x.to(device), f0.to(device), pitch.to(device))
                            y_mel = y_mel.to(device, non_blocking=True)
                            if USE_ALT_MELCALC:
                                y_g_hat_mel = alt_melspec(y_g_hat.squeeze(1))
                            else:
                            ...
                            if j <= 4:
                                if steps == 0:
                                    ...
                                    sw.add_figure('gt/y_spec_{}'.format(j), plot_spectrogram(y_mel.squeeze(0).cpu().numpy()), steps)
...
def main():
    ...
    build_env(a.config, 'config.json', a.ckpt_dir)
